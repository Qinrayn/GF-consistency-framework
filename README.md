...
| GAT | 0.069 | 0.533 | 0.160 |
| GraphSAGE | 0.069 | 0.531 | 0.177 |
| VGAE | 0.066 | 0.491 | 0.227 |

*G-F Score and AUC/F1 sourced from `results/gf_scores.json` and `results/metric_comparison.json` (11 methods, same run).*

- Community-detection baseline purity (4 algorithms, same purity formula as G-F curve): **greedy_modularity 0.128 · Leiden 0.180 · Louvain 0.180 · label_propagation 0.182** (range [0.128, 0.182])
- **Apples-to-apples comparison:** using the *same* algorithm the G-F pipeline uses (`greedy_modularity_communities`), the best embedding **Spectral (0.163) exceeds the baseline (0.128)** — the embedding recovers functional structure that single-shot community detection on the raw PPI does not. The higher Leiden/Louvain values (0.180) reflect a *stronger clustering algorithm*, not a stronger baseline; the Step 44 ablation confirms rankings are stable across all five algorithms (Kendall W = 0.797, Spectral #1 under 4/5).
- **2D is information-lossy:** even so, 2D compresses modern methods. Dimension sweep of Spectral embeddings shows function-prediction MRR rises from 0.066 (d=2) to a peak of 0.244 (d=512) — **surpassing the PPI-Neighbors baseline (0.219) at d ≥ 256** (d=128 gives 0.216, still below baseline; d=1024 drops back to 0.208). The embedding space also reveals functional structure *invisible* to community detection ("functional dark matter" — protein pairs unconnected in STRING but predicted by embedding proximity). See "High-Dimensional Analysis" below.
...
- Spearman rho (G-F Score vs Link Pred AUC, n=11): **+0.591** (*P* = 0.056, FDR *P* = 0.080, 95% CI [−0.09, 0.88]) — not significant at n=11. **Extended to n=25**: **rho = +0.750** (*P* < 0.001, FDR *P* < 0.001) — **significant after FDR correction**; see [`results/extended_25method_correlation.json`](results/extended_25method_correlation.json)
- Spearman rho (G-F Score vs k-NN F1, n=11): **+0.609** (*P* = 0.047, FDR *P* = 0.076, 95% CI [−0.05, 0.92]) — not significant at n=11. **Extended to n=25**: **rho = +0.725** (*P* < 0.001, FDR *P* < 0.001) — **significant after FDR correction**
- Spearman rho (G-F Score vs Function Prediction MRR): **+0.646** (*P* = 0.032, permutation *P* = 0.041, FDR-adjusted *P* = 0.069, n = 11) — not significant after FDR correction at n=11; however, with n=25 the GF-AUC and GF-F1 correlations are both highly significant (P<0.001), providing convergent evidence that GF Score predicts downstream utility
- Spearman rho (standard vs IC-weighted purity): **ρ = +0.964** (*P* < 0.001, FDR-adjusted *P* < 0.001) — IC-weighting preserves rankings while suppressing DAG inflation; **significant after FDR correction**
- Spearman rho (standard vs semantic purity): **ρ = +0.491** (*P* = 0.125) — semantic (Resnik) captures complementary coherence signal
- H1 max persistence vs G-F Score: **ρ = +0.764** (*P* = 0.006, FDR-adjusted *P* = 0.020, 95% CI [0.27, 0.97]) — **significant after FDR correction**
- Cross-species rank consistency (yeast vs human, 11 methods): **ρ = +0.491** (*P* = 0.125, FDR-adjusted *P* = 0.148), **Kendall W = 0.745** — moderate concordance; top-2 set {Spectral, MDS} consistent across species, MDS #1 on human
- **Multiple comparison correction** (Benjamini-Hochberg FDR, 13 tests at n=11): 4/13 significant after correction — IC-weighted consistency (ρ=+0.991), effective rank (ρ=+0.873), degree-embedding (ρ=−0.791), H1 persistence (ρ=+0.764). GF Score vs downstream metrics (AUC, F1, MRR) were not individually significant at n=11. **Extended to n=25**: GF vs AUC rho=+0.750 (P<0.001, FDR P<0.001), GF vs F1 rho=+0.725 (P<0.001, FDR P<0.001) — both now **highly significant after FDR correction**; see [`results/extended_25method_correlation.json`](results/extended_25method_correlation.json)
...
- GAT collapse diagnosis (5 variants): all variants show persistent 2D collapse — architectural constraints at d=2, not optimization-related. Note: at d=64, GAT achieves GF=0.260 (vs 0.069 at d=2), indicating the 2D output layer is a significant bottleneck
...
- **GAT collapse mechanism** (Phase 4): Attention degeneration (normalized entropy=0.973) renders GAT equivalent to GCN at d=2; 2-layer mean aggregation with 2D bottleneck produces rank-1 output (effective rank=1.019, dimension variance ratio=105:1). All 5 architectural variants fail at d=2. However, at d=64 GAT achieves GF=0.260 (3.8x improvement), confirming the collapse is primarily a 2D output constraint, not a fundamental architectural failure
...
- **GAT collapse analysis** (Phase 6): Three results provide theoretical motivation and empirical evidence for the collapse causal chain — (T1) attention degeneration bound from degree CV (the bound constant is data-derived, making this an empirical observation rather than a strict theorem), (T2) effective rank bound for mean-aggregation GNN (GNN: 1.045 vs non-GNN: 1.702, rho=0.873; a heuristic argument), (T3) G-F Score upper bound for rank-1 embeddings (rank-1 methods: GF_2D/GF_1D ratio~1.0; full-rank: up to 1.73; a numerical observation, not a strict theorem). Combined corollary: GAT collapse is consistent with architectural constraints, supported by empirical evidence across 5 variants and GATv2
...
- **Spectral Transferability Heuristic** (Phase 11): Proposes Spectral Quality Index (SQI = λ₂/λ₂_ER × PR × FA_max) as a heuristic indicator of when the two-factor model works. SQI ordering: yeast(10.72) > human(2.02) > mouse(0.54) matches two-factor rho: +0.929 > +0.483 > −0.037. Mouse Fiedler vector is 6× more localized (PR=0.0007). Validated on 20 SBM networks (SA_std vs log(SQI) rho=+0.647). Note: SQI is an empirical heuristic, not a formally derived theorem; the 3-species fit has no statistical power, and SBM validation uses ground-truth communities
...
- **E. coli K-12 4th Species Validation** (Step 40): MDS ranks #1 (GF=0.233), Spectral #2 (GF=0.229) on *E. coli* STRING network. SQI=0.7 (lowest among 4 species). Kendall's W=0.652 including *E. coli* (vs 0.739 for 3 eukaryotic species). Spectral-method family (Spectral or MDS) maintains top-2 in prokaryotes, with MDS edging ahead at low SQI
...
- **Degree-Controlled Pair-Level G-F** (Step 42c): Pair-level partial Spearman correlation of embedding distance and GO Jaccard similarity, controlling for degree dissimilarity. On the 153-node yeast network (11,628 pairs), the degree-annotation confound is confirmed: proteins with similar degrees share more GO terms (rho = -0.186, P = 2.2e-91). After degree control, only 3/11 methods retain significant geometry-function signal (permutation test, 200 permutations): MDS (rho = -0.370, P < 0.005), Spectral (rho = -0.367, P < 0.005), DM (rho = -0.118, P < 0.005). The top-3 ranking {Spectral, MDS, DM} is stable, but degree accounts for 24-56% of their pairwise correlation. PCA loses 88% of its signal (rho: -0.185 -> -0.022, P = 1.0). Random-walk methods (DeepWalk, Node2Vec) show no pair-level signal at all (rho ~ 0), confirming their G-F Scores arise from community detection rather than global distance-similarity correlation. Cross-species extension to human (500-node subsample, 124,750 pairs) and mouse (500-node subsample) reveals that MDS is the only method with consistent negative rho(D,S|degree) across all three species (yeast -0.370, human -0.042, mouse -0.163). Spectral loses significance on human (rho = -0.038, P = 1.0) where the degree-annotation confound is reversed (rho = +0.016). Mouse results are confounded by DAG-propagated GO annotations (17.1 terms/protein vs yeast 3.8), inflating GO Jaccard (51.3% pairs sharing GO vs 18.7% for yeast) and producing sign reversals for 5/11 methods. Cross-species rank concordance is low (Kendall W = 0.48). See [`results/degree_controlled_gf.json`](results/degree_controlled_gf.json) and [`results/degree_controlled_gf_multispecies.json`](results/degree_controlled_gf_multispecies.json)
- **G-F Conditional Embedding** (Step 42d): Optimizes a 2D embedding to directly maximize a differentiable proxy of the G-F Score (soft spatial weights + neighborhood GO purity), then evaluates it on independent metrics. The GF-optimized embedding (initialized from Spectral, 300 iterations) achieves GF=0.1637, marginally exceeding Spectral (0.1633, +0.25%). Critically, its degree-controlled rho(D,S|degree) = -0.3674, nearly identical to Spectral (-0.3669) and slightly worse than MDS (-0.3698). This means the G-F Score is NOT easily gameable: optimizing for G-F does not produce an embedding that cheats the metric without also achieving genuine degree-independent geometry-function signal. Spectral is near-optimal for G-F, and the framework is robust to adversarial optimization. See [`results/gf_conditional_embedding.json`](results/gf_conditional_embedding.json)
results/gf_conditional_embedding.json)
- **Mouse GO Annotation Fix** (Step 42e): Rebuilt mouse GO annotations from raw MGI GAF file (direct, non-propagated). Mean terms per protein drops from 17.1 (DAG-propagated) to 8.0 (direct), median from 12 to 4. However, the positive rho(D,S|degree) for mouse Spectral (+0.136) persists with direct annotations, confirming it reflects Fiedler vector localization (5/16180 nodes carry 50% of Fiedler energy) rather than a DAG artifact. This strengthens the Phase 11 SQI analysis. See [`data/mouse_go_annotations_direct.json`](data/mouse_go_annotations_direct.json)
- **NMI/ARI Metric Comparison** (Step 42f): Computes Normalized Mutual Information and Adjusted Rand Index between spatial-graph communities (at peak purity radius) and GO-based ground-truth partitions. **G-F Score vs ARI: rho=+0.691 (P=0.019, n=11)** - significant without n=25 extension. G-F Score vs NMI: rho=+0.646 (P=0.032). Top-3 overlap G-F vs ARI = 3/3 (Spectral, DM, MDS). MDS is #1 by both NMI and ARI but #3 by G-F Score, suggesting G-F slightly over-ranks Spectral. This is the first downstream metric correlation significant at n=11, addressing the statistical power concern. See [`results/metric_comparison_nmi_ari.json`](results/metric_comparison_nmi_ari.json)
results/metric_comparison_nmi_ari.json)
- **Dark Matter Disease Enrichment** (Step 42g): Maps the 44 yeast dark matter protein pairs to human functional orthologs via GO BP term overlap (no external API). Of 71 dark matter proteins, 18 have SGD GO annotations; 10 find human orthologs sharing >= 2 GO terms. These 10 orthologs are **7.4x enriched for disease-associated GO terms** (7/10 vs 4024/16818 background, Fisher exact p=0.003). This provides computational validation that embedding-predicted functional associations involve disease-relevant genes. Coverage is limited by SGD annotation depth on peripheral proteins. See [`results/dark_matter_disease_enrichment.json`](results/dark_matter_disease_enrichment.json)
results/dark_matter_disease_enrichment.json)
- **v11.5 Full-Network G-F + STRING Version Sensitivity** (Step 42h): Six methods with v11.5 full-network embeddings (5936 nodes) confirm that **Spectral ranks #1 on both 153-node (0.163) and full-network (0.194)**, demonstrating robustness to network scale. GNN methods benefit dramatically from full-network context: GIN (0.122 -> 0.193, +0.071), VGAE (0.066 -> 0.144, +0.078), GAT (0.069 -> 0.120, +0.050), confirming the 2D bottleneck hypothesis. VGAE-feat degrades (0.124 -> 0.067, -0.057), suggesting overfitting to the 153-node subgraph. Separately, STRING v12.0 embeddings (5716 nodes) produce a different ranking (PCA > Node2Vec > Spectral), but three-way comparison shows this is a version artifact (-0.020 version effect vs +0.031 scale effect). See [`results/v11_full_network_gf.json`](results/v11_full_network_gf.json) and [`results/full_network_all11.json`](results/full_network_all11.json)
results/full_network_all11.json)
- **Full GO Coverage + GO Aspect Dependency** (Step 42i): Parsed full SGD GAF to build a 5772-gene GO BP map (92.2% network coverage, vs 154 genes / 2.6% in the curated subset). True full-network G-F on 478-node subsample (STRING v12.0, server-verified): **Spectral GF=0.705** (vs 0.173 with 154-gene GO map), a 4x increase. With full GO coverage, Spectral ranks #1 again (0.705 > MDS 0.448 > DeepWalk 0.348 > Node2Vec 0.287 > PCA 0.183 > DM 0.114). The previous PCA>Spectral ranking on v12.0 was entirely a 154-gene GO map artifact - PCA benefited from sparse annotations. On 153-node curated network (local), Spectral G-F with full GO = 0.645. GO aspect dependency test (BP vs MF vs CC, 6 methods): G-F rankings are NOT robust across aspects - Spectral is #1 on BP but #4-5 on MF/CC; DM is #1 on MF/CC. BP vs MF rho=+0.43 (p=0.40), BP vs CC rho=-0.03 (p=0.96). Different embeddings capture different functional organization levels (BP=global community, MF=local activity, CC=localization). See [`results/full_go_and_aspect_test.json`](results/full_go_and_aspect_test.json) and [`results/go_aspect_dependency_test.json`](results/go_aspect_dependency_test.json)
...
- **GNN High-Dimensional GPU Training** (Step 47b): A800 GPU training of VGAE, GraphSAGE, GAT, GIN at d = {8, 16, 32, 64}. Key finding: **VGAE at d=8 achieves GF=0.160 via PCA-2d projection, nearly matching Spectral d=2 (0.163)**. GraphSAGE PCA-2d reaches 0.148 at d=8. The 2D output layer is a bottleneck — high-dimensional GNN embeddings capture functional structure that is lost in the final 2D projection. GIN peaks at d=32 (GF=0.125). GAT improves to 0.109 at d=32 but attention degeneration persists. Reframes the GNN evaluation: low d=2 GF Scores reflect output dimensionality constraints, not necessarily architectural failure
- **Functional Dark Matter Mining** (Step 48): Identifies protein functional associations invisible to network topology (>= 5 hops apart, not STRING-connected at >= 700). Spectral embedding reveals 44 dark matter pairs among 71 proteins at the extreme network periphery (median degree 4 vs 21, p=2.2e-15). Extraordinary GO enrichment: retrograde transport (1809x), transmembrane transport (960x), ascospore formation (710x), ERAD pathway (521x). These are functional associations that NO network-based method could ever discover. **High-dimensional validation** (d=64, [`results/functional_dark_matter_d64.json`](results/functional_dark_matter_d64.json)): 64D Spectral embedding finds 35 pairs among 47 proteins — fewer but more selective, with 3 high-confidence pairs (vs 2 in 2D) and higher fold enrichment (174x vs 111x for top term GO:0045944, positive regulation of transcription). Both dimensions identify the same core functional signal (transcription regulation dark matter), confirming 2D findings are not dimension artifacts
...
- **Cheeger-Spectral G-F Bound** (v2.10.0, Step 65): Calibrated empirical GF upper bound motivated by Cheeger's inequality. The 4-component bound's parameters (8 coefficients) are fit via L-BFGS-B optimisation on 6 species networks — this is a calibrated empirical formula, not a theorem-derived bound. Spectral gap component B1 dominates (w1 = 0.999). Valid for all 6 networks. Tightness: Drosophila 0.996, Human 0.95, Yeast curated 0.44, Mouse 0.36, E. coli 0.28. LOO-CV rho = 0.77 across 6 species (n=6, limited stability). Enables pre-screening of network suitability for spectral analysis without computing any embedding
- Unified interval: **[0.05, 0.422]**
- **High-dimensional G-F Score theory** (Theorems 5-6): The Euclidean GF Score is systematically inflated in high dimensions due to distance concentration — Spearman rho(GF_euc, distance CV) = **−1.000** (p<0.001), with GF_euc rising 64% from d=2 (0.163, CV=0.81) to d=64 (0.267, CV=0.11) driven entirely by concentration, not embedding quality. The **kNN-GF Score** (Proposition 3) replaces distance thresholds with k-nearest-neighbour graphs, depending only on distance rankings. Theorem 5 proves kNN-GF is invariant to distance concentration; Theorem 6 proves Euclidean GF is inflated. Corollary: kNN-GF is the correct cross-dimensional metric. See [`results/highdim_knn_gf_comparison.json`](results/highdim_knn_gf_comparison.json) and Theorems 5-6 in [`Supplementary_Materials.txt`](Supplementary_Materials.txt)
...
Step 42c ─ Degree-Controlled G-F ─────────── Pair-level partial Spearman (3/11 significant)
...
Step 48 ─ Functional Dark Matter Mining ────── Embedding-only functional associations (44 pairs)
...
├── scripts/                    # 72-step analysis pipeline + extensions (110 active)
...
│   ├── interval_sensitivity_analysis.py    # Interval data-snooping risk assessment (P1-2)
│   ├── generate_manifest.py                # Auto-generate results/MANIFEST.md (P1-5)
│   ├── embed_extended_methods.py           # 14 new methods for n=25 statistical power
...
│   ├── degree_controlled_gf.py             # Degree-controlled pair-level G-F: partial Spearman test (Step 42c)
│   ├── degree_controlled_gf_multispecies.py # Cross-species degree-controlled G-F: yeast + human + mouse
│   ├── gf_conditional_embedding.py        # G-F optimized conditional embedding: framework stress test (Step 42d)
│   ├── fix_mouse_go_annotations.py        # Rebuild mouse GO from raw GAF: direct (non-propagated) annotations (Step 42e)
│   ├── metric_comparison_nmi_ari.py       # G-F Score vs NMI vs ARI: standard clustering metric validation (Step 42f)
│   ├── dark_matter_disease_enrichment.py # Dark matter -> human orthologs -> disease GO enrichment: 7.4x (p=0.003) (Step 42g)
│
...
> Legend: `*` = requires optional dependency (e.g. Ripser for persistent homology); `~` = archived (source in `scripts/_legacy/`, results retained in `results/`).
...
| `robustness_analysis.py` ~ | Convergence analysis, randomization null test, power curve estimation (archived; results retained) |
...
| `deep_geometric_analysis.py` ~ | Multi-scale geometric fingerprint: DFC, geometric features, curve decomposition (Phase 1, Fig 26-29) (archived; results retained) |
| `geometric_predictor.py` ~ | Cross-species geometric predictability: yeast→human validation, spectral theory (Phase 2, Fig 30-33) (archived; results retained) |
| `spectral_alignment.py` | Network-aware spectral alignment: Laplacian eigenbasis decomposition (Phase 3, Fig 34-35) |
| `gat_collapse_theory.py` ~ | GAT collapse mathematical theory: 4-pillar impossibility analysis (Phase 4, Fig 36-38) (archived; results retained) |
...
| `gat_dimension_sweep.py` ~ | GAT latent dimension sweep d={2,4,8,16,32}: causal disentanglement of attention degeneration (Phase 5B, Fig 41) (archived; results retained) |
| `gat_collapse_formal_proof.py` ~ | Formal proofs of GAT collapse: 3 theorems with numerical verification — attention degeneration bound, effective rank bound, G-F Score upper bound (Phase 6, Fig 42-43) (archived; results retained) |
...
| `mouse_data_prep.py` ~ | Mouse STRING PPI + MGI GAF download with Ensembl_MGI alias-based ID mapping: ~16K nodes, ~233K edges, 17,639 annotated genes (Phase 10A) (archived; data retained) |
| `mouse_embeddings_full.py` ~ | Full-network mouse embeddings (11 methods, ~16K nodes) with landmark MDS + sparse VGAE/GNN — matches human pipeline methodology (subsample-after, not subsample-before) (Phase 10B) (archived; embeddings retained) |
...
| `biological_validation.py` ~ | Biological validation + statistical power: (A) GO BP hypergeometric enrichment across 3 species × 11 methods; (B) multi-seed panel (yeast 5 seeds, human 10, mouse 5 subsamples) with mixed-effects pooled Spearman model; `part_a`/`part_b` CLI modes with checkpoint resume (Phase 12, Fig 60-64) (archived; results retained) |
...
| `degree_controlled_gf.py` * | Degree-controlled pair-level G-F: partial Spearman of embedding distance vs GO Jaccard, controlling for degree dissimilarity; 3/11 methods retain significant signal (Step 42c) |
| `degree_controlled_gf_multispecies.py` * | Cross-species degree-controlled G-F: yeast + human + mouse, 500-node subsamples; MDS is the only method with consistent negative rho across all 3 species (Step 42c) |
| `gf_conditional_embedding.py` * | G-F conditional embedding: optimizes a differentiable G-F proxy, then tests on degree-controlled rho; GF-opt achieves 0.1637 (near-Spectral), framework is not gameable (Step 42d) |
| `fix_mouse_go_annotations.py` * | Rebuild mouse GO from raw MGI GAF (direct, non-propagated); mean terms 17.1 -> 8.0; confirms mouse Spectral anomaly is Fiedler localization, not DAG artifact (Step 42e) |
| `metric_comparison_nmi_ari.py` * | G-F Score vs NMI vs ARI: rho(GF, ARI) = +0.691 (P=0.019, n=11), significant without n=25 extension; top-3 overlap 3/3 (Step 42f) |
| `dark_matter_disease_enrichment.py` * | Dark matter disease enrichment: 10 human orthologs 7.4x enriched for disease GO terms (Fisher p=0.003) (Step 42g) |
...
| `multihead_gat_experiment.py` ~ | Multi-head GAT configuration sweep (1/4/8 heads, d=2-32) (archived; results retained) |
...
The community-detection baseline (apples-to-apples `greedy_modularity`, the same algorithm the G-F pipeline uses) is **0.128**; Leiden/Louvain reach 0.180 with a stronger clustering algorithm. Either way, at d=64 the embeddings dominate: **four methods exceed every baseline**: MDS (0.267), Spectral (0.267), GIN (0.261), GAT (0.260) — all above even the strongest baseline (Leiden 0.180).
...
- **2D output limitation**: The primary G-F rankings are based on 2D embeddings (standardized to σ = 0.3), which is information-lossy for methods designed for higher dimensions. Under the *same* algorithm the G-F pipeline uses (`greedy_modularity`), the best 2D embedding (Spectral: 0.163) already exceeds the baseline (0.128); the stronger Leiden/Louvain algorithms reach 0.180 on the raw PPI, and the Step 44 ablation confirms G-F rankings are stable across all five community-detection algorithms (Kendall W = 0.797). Dimension sweep of Spectral embeddings shows function-prediction MRR rises from 0.066 (d=2) to a peak of 0.244 (d=512), surpassing the PPI-Neighbors baseline (0.219) at d ≥ 256 (d=1024 drops back to 0.208). The 2D results remain the primary metric for interpretability and visualisation, but higher-dimensional analysis is essential for fair evaluation of modern methods.
- **GNN dimensionality**: GraphSAGE, GAT, GIN, and VGAE use a 2D bottleneck (latent_dim = 2) in the primary analysis. GPU training (A800) at d = {8, 16, 32, 64} reveals that the 2D output layer is a significant bottleneck: VGAE at d=8 achieves GF = 0.160 via PCA-2d projection (nearly matching Spectral d=2 = 0.163), and GraphSAGE reaches 0.148. The raw 2d slice scores remain low (~0.06–0.10), but PCA projection of high-dimensional embeddings recovers hidden functional structure. GIN peaks at d=32 (GF = 0.125). GAT improves to 0.109 at d=32 but attention degeneration persists across all dimensions, confirming the GAT collapse is partially architectural.
...
- **Spearman correlations and multiple comparison correction**: At n=11, G-F Score vs AUC (ρ=0.591, P=0.056), vs F1 (ρ=0.609, P=0.047), and vs MRR (ρ=0.646, P=0.032) were not significant after BH FDR correction (13 tests; FDR-adjusted P = 0.080, 0.076, 0.069). Four tests survived: IC-weighted (ρ=+0.991), effective rank (ρ=+0.873), degree-embedding (ρ=−0.791), H1 persistence (ρ=+0.764). **Method panel expanded to n=25** (adding Laplacian-EM, Isomap, LLE, Mod-LLE, KPCA, tSNE-2D, RandProj, DegreeEnc, Spectral+, Spectral-, GraphFact, HeatKernel, PageRank, Gravity): **GF vs AUC rho=+0.750 (P<0.001, FDR P<0.001), GF vs F1 rho=+0.725 (P<0.001, FDR P<0.001)** — both now highly significant after FDR correction. The n=11 limitation is resolved; n=25 provides adequate statistical power. Note: of the 14 new methods, Laplacian-EM is mathematically identical to Spectral (same GF=0.163), and Spectral+/Spectral- are eigenvector variants of the same method; excluding these 3 variants leaves n=22 independent methods, still adequate for significance. See [`results/extended_25method_correlation.json`](results/extended_25method_correlation.json).
- **Degree-annotation confound**: Pair-level analysis (Step 42c) confirms that proteins with similar degrees share more GO terms (Spearman rho = -0.186, P = 2.2e-91, 11,628 pairs). After controlling for degree dissimilarity via partial Spearman correlation, only 3/11 methods retain significant geometry-function signal (MDS, Spectral, DM; permutation P < 0.005). Degree accounts for 24-26% of the MDS/Spectral signal and 56% of DM; PCA loses 88%. The top-3 ranking is stable, but 8/11 methods - including all random-walk and GNN methods - show no pair-level signal, indicating their G-F Scores arise from community detection and degree encoding rather than global distance-similarity correlation. Cross-species extension to human (500-node subsample) and mouse (500-node subsample) shows that MDS is the only method with consistent negative rho(D,S|degree) across all three species. Spectral loses significance on human (P = 1.0). Mouse results are confounded by DAG-propagated GO annotations (17.1 terms/protein vs yeast 3.8), limiting cross-species comparability. Cross-species rank concordance is low (Kendall W = 0.48). See [`results/degree_controlled_gf.json`](results/degree_controlled_gf.json) and [`results/degree_controlled_gf_multispecies.json`](results/degree_controlled_gf_multispecies.json)
- **GAT-family embedding collapse at 2D**: GAT exhibits persistent 2D embedding collapse across species, architectures, and attention variants. Five architectural variants (gradient clipping, warmup, multi-head attention) and GATv2 dynamic attention (Brody et al., ICLR 2022) have been tested at d=2 — GATv2 partially reduces attention entropy (0.903 vs 0.927) but does not rescue 2D G-F Score (0.157 vs 0.154, both near-random). However, at d=64 GAT achieves GF=0.260 (3.8x improvement over d=2), confirming that the 2D output layer is the primary bottleneck, not the attention mechanism itself. The 2D collapse is driven by the combination of attention degeneration and the 2D output constraint on degree-heterogeneous PPI networks.
...
- **Network scale and STRING version sensitivity** (Step 42h): v11.5 full-network G-F (5936 nodes, 6 methods) confirms Spectral ranks #1 on both scales (153-node: 0.163, full: 0.194). GNN methods improve with full-network context (GIN +0.071, VGAE +0.078, GAT +0.050), confirming the 2D bottleneck. VGAE-feat degrades (-0.057), suggesting 153-node overfitting. STRING v12.0 (5716 nodes) produces a different ranking, but three-way comparison shows this is a version artifact (version effect -0.020 < scale effect +0.031). Only 6/11 methods available on v11.5 full network (DM/MDS/Node2Vec/PCA/DeepWalk were overwritten by v12.0 server output). See [`results/v11_full_network_gf.json`](results/v11_full_network_gf.json)
...
- **Embedding outliers**: Outlier sensitivity analysis ([`results/outlier_sensitivity_analysis.json`](results/outlier_sensitivity_analysis.json)) reveals that 8/11 human embedding methods have at least one node exceeding 10σ deviation. Most extreme: DeepWalk ENSP00000248150 (64σ), Node2Vec same node (56σ), Spectral ENSP00000398559 (47σ). The original `outlier_report.txt` (from deprecated `human_validation/human_embed_all.py`) only detected ENSP00000334051 in DeepWalk/Node2Vec; current embeddings (from `human_embed_extended.py`) show different outlier patterns. G-F Score's community-level aggregation provides some robustness to individual outliers, but human rankings should be interpreted with this caveat. Future work should report G-F Scores with and without outlier removal.
- **Integration interval selection**: The unified interval [0.05, 0.422] was determined data-adaptively (Step 16, `adaptive_interval.py`). To rule out data snooping, interval sensitivity analysis ([`results/interval_sensitivity_analysis.json`](results/interval_sensitivity_analysis.json)) tests 7 pre-specified intervals: Spectral ranks #1 in all 7/7 intervals (100% top-1 consistency). Minimum Spearman rho vs current interval = 0.891 (offset [0.20, 0.422]), all p < 0.001. Rankings are robust to interval choice — the unified interval does not artificially inflate any method's ranking.
...
Y.Z. conceived the study, designed the G-F consistency framework, implemented all 11 embedding methods and 72-step analysis pipeline, performed the cross-species validation (5 species), derived the mathematical propositions, conducted the functional dark matter analysis, wrote all scripts (110 active + 16 legacy + 7 deprecated human validation = 133 total), generated all figures, and wrote the manuscript.
