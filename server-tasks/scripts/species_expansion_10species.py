...
SUBSAMPLE_SIZE = 500   # for large networks (keeps G-F computation tractable)
...
SPECIES_REGISTRY = {
    "Yeast":      {"taxon": "4932",   "string_file": "4932.protein.links.v12.0.txt.gz"},
    "Human":      {"taxon": "9606",   "string_file": "9606.protein.links.v12.0.txt.gz"},
    "Mouse":      {"taxon": "10090",  "string_file": None},  # edgelist
    "Ecoli":      {"taxon": "511145", "string_file": "511145.protein.links.v12.0.txt.gz"},
    "Fly":        {"taxon": "7227",   "string_file": "7227.protein.links.v12.0.txt.gz"},
    # New species
    "Celegans":   {"taxon": "6239",   "string_file": "6239.protein.links.v12.0.txt.gz"},
    "Athaliana":  {"taxon": "3702",   "string_file": "3702.protein.links.v12.0.txt.gz"},
    "Zebrafish":  {"taxon": "7955",   "string_file": "7955.protein.links.v12.0.txt.gz"},
    "Rat":        {"taxon": "10116",  "string_file": "10116.protein.links.v12.0.txt.gz"},
    "Chicken":    {"taxon": "9031",   "string_file": "9031.protein.links.v12.0.txt.gz"},
}
...
def load_go_annotations(species_name, nodes):
    ...
    json_files = {
        "Yeast": data_dir / "gene_go_map_full.json",
        "Human": data_dir / "human_go_annotations.json",
        "Mouse": data_dir / "mouse_go_annotations_direct.json",
    }

    # Fall back to curated 154-gene map if full map not built yet
    if species_name == "Yeast" and not json_files["Yeast"].exists():
        json_files["Yeast"] = data_dir / "gene_go_map.json"
    if species_name == "Mouse" and not json_files["Mouse"].exists():
        json_files["Mouse"] = data_dir / "mouse_go_annotations.json"
    ...
    gaf_files = {
        ...
        "Ecoli": data_dir / "new_species" / "go" / "511145.gaf.gz",
        "Fly": data_dir / "new_species" / "go" / "7227.gaf.gz",
    }
...
def main():
    ...
    # ----------------------------------------------------------------
    # Load existing species results (if available; otherwise compute)
    # ----------------------------------------------------------------
    print("[1/3] Loading existing species results ...")
    results_dir = get_results_dir()

    existing_species = ["Yeast", "Human", "Mouse", "Ecoli", "Fly"]
    loaded_existing = 0
    ...
    for species in existing_species:
        ...
        try:
            if species == "Yeast":
                ...
                spectral = float(gf_data["scores"].get("Spectral", 0.0))
                best = max(gf_data["scores"].values())
            elif species == "Human":
                for fname in ["human_gf_scores_extended.json", "human_gf_scores.json"]:
                    ...
                    raise FileNotFoundError("No human GF file")
            elif species == "Mouse":
                md = json.load(open(results_dir / "mouse_gf_analysis.json"))
                raw = md.get("gf_scores", {})
                spectral = float(raw.get("Spectral", {}).get("gf_score", 0.0))
                best = max(float(v["gf_score"]) for v in raw.values() if isinstance(v, dict))
            elif species == "Ecoli":
            ...
            all_results.append({
                "species": species, "gf_score_spectral": float(spectral),
                "gf_score_best": float(best), "source": "existing",
            })
            print(f"  {species}: Spectral GF={spectral:.4f} (from existing file)")
            loaded = True
            loaded_existing += 1
        except Exception:
            print(f"  {species}: no existing result file, will compute from scratch")

    print(f"  Loaded {loaded_existing} existing species")
    print()

    # ----------------------------------------------------------------
    # Compute missing species (existing ones that failed + all new)
    # ----------------------------------------------------------------
    print("[2/3] Computing species from scratch ...")
    species_to_compute = [s for s in existing_species
                          if s not in [r["species"] for r in all_results]]
    species_to_compute += ["Celegans", "Athaliana", "Zebrafish", "Rat", "Chicken"]

    for species in species_to_compute:
        print(f"\n  [{species}]")
        t0 = time.time()
        try:
            ...
            print(f"    Skipping {species}, continuing with next species")
