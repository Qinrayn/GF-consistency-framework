...
def load_full_STRING_network(data_dir: Optional[Path] = None,
    ...
    """Load the full yeast STRING network (score >= *min_score*).

    Tries v12.0 first, falls back to v11.5.
    """
    if data_dir is None:
        data_dir = get_data_dir()
    data_dir = Path(data_dir)
    string_file = data_dir / "4932.protein.links.v12.0.txt.gz"
    if not string_file.exists():
        string_file = data_dir / "4932.protein.links.v11.5.txt.gz"
    if not string_file.exists():
        # Try edgelist fallback
        ...
        if edgelist.exists():
            ...
            return G
        raise FileNotFoundError(f"STRING file not found in {data_dir}")
